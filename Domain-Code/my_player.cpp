...
my_player::my_player(My_Q_Table &q_table_, double alpha, double gamma){

    // Assign Q-table
    q_table = &q_table_;
    set_learning_rate(alpha);
    set_discount_factor(gamma);
...
// my_player::my_player(My_Q_Table &q_table_){
...
void my_player::set_discount_factor(double value){
    if(value < 0.0 || value > 1.0)
        throw std::exception();
...
//    Find best available moves in the current position    
    ...
    for (int i = 0; i < available_count; i++)
    {
        ...
        best_action = predict_action(piece_current); 

        q_val = q_table->get_q_table_value(states, best_action); 

        if (q_val > best_val){ // ************** Possible bug from here actions are not stored correctly...
            best_val = q_val;
...
void my_player::set_learning(bool toggle_val){
    learning_on = toggle_val;
    q_table->set_qtable_learning(toggle_val);
    //std::cout << "Learning is now: " << learning_on << std::endl;
}
